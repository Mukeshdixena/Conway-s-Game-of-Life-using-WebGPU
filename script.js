async function checkWebGPU() {
  const warningBox = document.getElementById("webgpu-replacement");
  ...
  function showWarning() {
    ...
    warningBox.innerHTML = `
        <div class="webgpu-msg">
            <h2>WebGPU Not Enabled</h2>

            <p>Go to your browser's flag settings by opening the following URL based on your browser:</p>

            <code>chrome://flags/</code><br>
            <code>brave://flags/</code>

            <p>Then enable:</p>
            <strong>Unsafe WebGPU Support</strong>
        </div>
    `;
  }
  ...
  try {
    ...
    if (!adapter) return showWarning();

    const device = await adapter.requestDevice();
    if (!device) return showWarning();
